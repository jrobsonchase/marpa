...
impl Lexer {
    ...
    fn new_token(&mut self, end: usize, ty: i32) -> Token {
        let val = self.tokens.len();
        let start = if val > 0 { self.tokens.last().unwrap().end } else { 0 };
        self.tokens.push(Token{ start: start, end: end, value: val as i32, ty: ty });
    ...
    pub fn next_token_hinted<I: IntoIterator<Item=usize>>(&mut self, it: I) -> Option<Token> {
        for ix in it {
